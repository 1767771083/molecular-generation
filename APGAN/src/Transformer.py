...
class LayerNormalization(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # eg.for (batch, node_num, feature_dim) inputs, mean -> (batch, node_num, 1)
        var = ((inputs - mean) ** 2).mean(dim=dims, keepdim=True)  # (batch, node_num, 1)
        std = (var + self.eps).sqrt()  # (batch, node_num, 1)
