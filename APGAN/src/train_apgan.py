...
def train(args):
    ...
    print('==========================================')
    logging.info('Load {} data done! Time {:.2f} seconds'.format(args.data_name, time.time() - start))
    logging.info('Max Epoch: {}'.format(args.max_epochs))
    logging.info('Num Iter/Epoch: {}'.format(len(train_dataloader)))
    logging.info('Batch size: {}'.format(args.batch_size))
    logging.info('Weight of property: {}'.format(args.attr_weight_1))
    print('==========================================')
    ...
    discriminator_optimizer = torch.optim.Adam(discriminator.parameters(),
                                               lr=1e-4)
    predictor_optimizer = torch.optim.Adam(predictor.parameters(), lr=1e-6)
    ...
    for epoch in range(args.max_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            label_pic = []
            if len(real_label[1]) > 2:
                pic_max = real_label[:, 2].max()
                pic_min = real_label[:, 2].min()
                label_pic = [pic_min.cpu().numpy(), pic_max.cpu().numpy()]

            graph_encoder_output = graph_encoder(x, adj)

            fake_label = get_fake_label(args.label_qed, args.label_sa, label_pic, size=(x.shape[0], 1), device=device)
            ...
            for _ in range(args.n_critic):
                ...
                pred_real, real_modified_features, _ = predictor(graph_encoder_output)
                pred_fake, fake_modified_features, _ = predictor(generator_output)
            ...
            # pred_pic = torch.mean(pred_fake[:, 2])

            property_mean = args.weight_property * pred_qed + (1 - args.weight_property) * pred_sa

            generator_loss = -torch.mean(fake_discriminator_outputs) + args.attr_weight_1 * (1.0 - property_mean)
            ...
            predictor_loss_real = qed_predictor_loss_real + sa_predictor_loss_real
            # + pic_predictor_loss_real
            ...
            predictor_loss_fake = qed_predictor_loss_fake + sa_predictor_loss_fake
            # + pic_predictor_loss_fake

            # predictor_loss = 0.8 * predictor_loss_real + 0.2 * predictor_loss_fake

            predictor_loss = 0.8 * qed_predictor_loss_real + 0.2 * qed_predictor_loss_fake
        ...
        #                os.path.join(args.model_dir, '{}_discriminator.pth'.format(args.data_name)))
        #     torch.save(graph_encoder.state_dict(),
        #                os.path.join(args.model_dir, '{}_graph_encoder.pth'.format(args.data_name)))
        #     torch.save(latent_decoder.state_dict(),
        #                os.path.join(args.model_dir, '{}_latent_decoder.pth'.format(args.data_name)))
        ...
        def print_validity(ith):
            with torch.no_grad():
                ...
                logging.info(
                    ...
                    "Mean QED: {:.3f}, Mean SA: {:.3f}".format(
                        valid_res['valid_ratio'],
                        valid_res['unique_ratio'],
                        valid_res['abs_unique_ratio'],
                        novel_r,
                        abs_novel_r,
                        np.mean(eval_smiles_qed) if eval_smiles_qed else 0.0,
                        np.mean(eval_smiles_sa) if eval_smiles_sa else 0.0
                    )
                )
    ...
    logging.info("[Training Ends], Start at {}, End at {}".format(
    ...
    def test():
        ...
        generator.load_state_dict(torch.load(os.path.join(args.model_dir, '{}_generator.pth'.format(args.data_name))))
        latent_decoder.load_state_dict(
        ...
        with open(f'{args.result_dir}/{args.attr_weight_1}_{args.data_name}_generated_smiles.txt', 'w') as file:
        ...
        logging.info(
                    'test max QED:{:.3f}\n'
                    'test mean QED:{:.3f}'.format(
                        np.max(test_smiles_qed),
                        np.mean(test_smiles_qed))
        )

        # test_smiles_sa = [calculate_sa_score(mol) for mol in valid_res['valid_mols']]
        # logging.info(
        #             'test min SA:{:.3f}\n'
        #             'test mean SA:{:.3f}'.format(
        #                 np.min(test_smiles_sa),
        #                 np.mean(test_smiles_sa))
        # )
        logging.info('===================================================')
        ...
        eval_dir = os.path.join(args.result_dir, args.test_subdirr, args.data_name)  # (args.result_dir, args.eval_subdir)
        if not os.path.exists(eval_dir):
            os.makedirs(eval_dir)

        # with open(os.path.join(eval_dir, f"{args.data_name}_{args.attr_weight_1}_test_results.txt"), "w") as file:
        with open(os.path.join(eval_dir, f"{args.data_name}_test_results.txt"), "w") as file:
        ...
        print("Results saved to {}_{}_test_results.txt".format(args.data_name, args.attr_weight_1))
