...
def parse():
    ...
    parser.add_argument('--data_name', type=str, default='moses10k',
                        choices=['qm9', 'zinc250k', 'moses10k'],
...
if data_name == 'qm9':
    ...
    print('Preprocessing moses10k data')
    df_moses10k = pd.read_csv('moses10k.csv', index_col=0)
    labels = ['QED']
    parser = DataFrameParser(preprocessor, labels=labels, smiles_col='SMILES')
    result = parser.parse(df_moses10k, return_smiles=True)
    dataset = result['dataset']
    smiles = result['smiles']
else:
    raise ValueError("[ERROR] Unexpected value data_name={}".format(data_name))
