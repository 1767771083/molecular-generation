...
moses10_atomic_num_list = [6, 7, 8, 9, 16, 17, 35, 0]  # 0 is for virtual node.
max_atoms = 26
n_bonds = 4


def one_hot_moses10k(data, out_size=26):
    num_max_id = len(moses10_atomic_num_list)
    assert data.shape[0] == out_size
    b = np.zeros((out_size, num_max_id), dtype=np.float32)
    for i in range(out_size):
        ind = moses10_atomic_num_list.index(data[i])
        b[i, ind] = 1.
    return b


def transform_fn_moses10k(data):
    node, adj, label = data
    # convert to one-hot vector
    # node = one_hot(node).astype(np.float32)
    node = one_hot_moses10k(node).astype(np.float32)
